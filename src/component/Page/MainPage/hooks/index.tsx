...
export default function MainPage() {
  ...
  const number = useValueInclude(false);
  const char = useValueInclude(false);
  ...
  useEffect(() => {
    const pw = generatePw();
    setPw(pw);
  }, [range, number.checked, char.checked]);
  ...
  function generatePw(): string {
    ...
    while (password.length < range) {
      if (number.checked && password.length < range) {
        password += numbers.charAt(Math.floor(Math.random() * numbers.length));
      }

      if (char.checked && password.length < range) {
      ...
      <div className="bg-slate-300 bg-opacity-40 w-1/2 flex flex-col p-5">
        ...
        <div className="flex flex-row justify-evenly">
          ...
          <div className="flex items-center">
            ...
            <input type="checkbox" id="checkbox" {...char} />
          </div>
        </div>
      </div>
    </div>
...
function useValueInclude(initialValue: boolean) {
    ...
    function handleValueChange() {
      setIsValueInclude((prev) => !prev);
    }

    return {
      checked: isValueInclude,
